...
[cookies module documentation](https://www.npmjs.org/package/cookies#readme):
  ...
  - `sameSite`: a boolean or string indicating whether the cookie is a "same site" cookie (`false` by default). This can be set to `'none'`, `'strict'`, `'lax'`, or `true` (which maps to `'strict'`). If this is set to `none`, you should also set `secure` to `true`, otherwise the sameSite option will not be respected by Chrome.
